...
```c++
typedef utils::workflow::t_module<utils::workflow::t_sort<int> > module_sort_t;
```
...
namespace utils{
  template <class _nt>
  class t_options<workflow::t_module<t_sort<_nt> > >
  {
  public:
    boost::program_options::options_description operator()(workflow::t_data<workflow::t_module<t_sort<_nt> > >& d)
    {
...
```c++
typedef utils::workflow::t_module<t_sort<int> > module_sort_t;
typedef utils::workflow::t_module<t_find<int> > module_find_t;
```

We now connect the modules so that we first ort, then find. This is done using **connectors**, in particular the *next* connector :

```c++
typedef utils::workflow::t_module<utils::workflow::t_next<module_sort_t, module_find_t> > module_t;
```
...
```c++
typedef utils::t_workflow<module_t>               workflow_t;
```
...
If necessary, we can also define an options manager that will handle the command-line options (note that the file [include/utils/workflow_options.hpp](include/utils/workflow_options.hpp) has to be included and the project has to be linked with the Boost library) :

```c++
typedef utils::t_workflow_options_manager<module_t>        options_manager_t;
```
